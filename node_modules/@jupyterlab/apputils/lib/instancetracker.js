 ...
 */
var InstanceTracker = /** @class */ (function () {
    /**
    ...
    Object.defineProperty(InstanceTracker.prototype, "widgetUpdated", {
        /**
    ...
    InstanceTracker.prototype.restore = function (options) {
        ...
        var promises = when ? [state.fetchNamespace(namespace)].concat(when)
            : [state.fetchNamespace(namespace)];
        this._restore = options;
        return Promise.all(promises).then(function (_a) {
            var saved = _a[0];
            return Promise.all(saved.map(function (item) {
                var id = item.id, value = item.value;
                var args = value && value.data;
                if (args === undefined) {
                    return state.remove(item.id);
                }
                // Execute the command and if it fails, delete the state restore data.
                return registry.execute(command, args).catch(function () { return state.remove(id); });
            }));
        });
    };
    /**
    ...
    InstanceTracker.prototype.save = function (widget) {
        ...
        var newName = widgetName ? this.namespace + ":" + widgetName : '';
