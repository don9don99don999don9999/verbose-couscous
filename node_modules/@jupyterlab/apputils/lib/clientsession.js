...
var ClientSession = /** @class */ (function () {
    ...
    Object.defineProperty(ClientSession.prototype, "kernelDisplayName", {
        ...
        get: function () {
            ...
            var specs = this.manager.specs;
            if (!specs) {
                return 'Unknown!';
            }
            var spec = specs.kernelspecs[kernel.name];
    ...
    ClientSession.prototype.initialize = function () {
        ...
        return manager.ready.then(function () {
            var model = algorithm_1.find(manager.running(), function (item) {
                return item.path === _this._path;
            });
            if (!model) {
                return;
            }
            return manager.connectTo(model).then(function (session) {
    ...
    ClientSession.prototype._selectKernel = function () {
        ...
        var dialog = this._dialog = new dialog_1.Dialog({
            title: 'Select Kernel',
            body: new Private.KernelSelector(this),
            buttons: [dialog_1.Dialog.cancelButton(), dialog_1.Dialog.okButton({ label: 'SELECT' })]
        });
        return dialog.launch().then(function (result) {
            ...
            if (model === null && _this._session) {
                return _this.shutdown().then(function () { return _this._kernelChanged.emit(null); });
            }
            if (model) {
                return _this._changeKernel(model).then(function () { return void 0; });
            }
        }).then(function () { _this._dialog = null; });
    };
    /**
    ...
    ClientSession.prototype._startSession = function (model) {
        ...
        return this.manager.startNew({
            ...
            return _this._handleNewSession(session);
        }).catch(function (err) {
            _this._handleSessionError(err);
    ...
    ClientSession.prototype._handleSessionError = function (err) {
        ...
        return err.response.text().then(function (text) {
            ...
            var dialog = _this._dialog = new dialog_1.Dialog({
                title: 'Error Starting Kernel',
                body: virtualdom_1.h.pre(message),
     ...
     */
    ClientSession.prototype._onPropertyChanged = function (sender, property) {
        switch (property) {
            case 'path':
...
(function (ClientSession) {
    ...
    function restartKernel(kernel) {
        var restartBtn = dialog_1.Dialog.warnButton({ label: 'RESTART ' });
...
(function (Private) {
     ...
     */
    var KernelSelector = /** @class */ (function (_super) {
        __extends(KernelSelector, _super);
        /**
         * Create a new kernel selector widget.
         */
        function KernelSelector(session) {
            return _super.call(this, { node: createSelectorNode(session) }) || this;
        }
        /**
         * Get the value of the kernel selector widget.
         */
        KernelSelector.prototype.getValue = function () {
            var selector = this.node.querySelector('select');
    ...
    function getDefaultKernel(options) {
        ...
        var name = preference.name, language = preference.language, shouldStart = preference.shouldStart, canStart = preference.canStart, autoStartDefault = preference.autoStartDefault;
        if (!specs || shouldStart === false || canStart === false) {
            return null;
        }
        var defaultName = autoStartDefault ? specs.default : null;
        if (!name && !language) {
            return defaultName;
        }
    ...
    function optionForSession(session, displayName, maxLength) {
        var option = document.createElement('option');
        var sessionName = session.name || coreutils_1.PathExt.basename(session.path);
