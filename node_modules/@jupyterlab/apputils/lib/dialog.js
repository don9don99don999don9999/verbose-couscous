...
var Dialog = /** @class */ (function (_super) {
    ...
    function Dialog(options) {
        ...
        var normalized = Private.handleOptions(options);
        var renderer = normalized.renderer;
        _this._host = normalized.host;
        _this._defaultButton = normalized.defaultButton;
        _this._buttons = normalized.buttons;
        ...
        _this._body = normalized.body;
        var header = renderer.createHeader(normalized.title);
        var body = renderer.createBody(normalized.body);
        var footer = renderer.createFooter(_this._buttonNodes);
        content.addWidget(header);
        content.addWidget(body);
        content.addWidget(footer);
        _this._primary = _this._buttonNodes[_this._defaultButton];
        if (options.focusNodeSelector) {
            var el = body.node.querySelector(options.focusNodeSelector);
            if (el) {
                _this._primary = el;
            }
        }
    ...
    Dialog.prototype.launch = function () {
        ...
        var promise = this._promise = new coreutils_1.PromiseDelegate();
        var promises = Promise.all(Private.launchQueue);
     ...
     */
    Dialog.prototype._resolve = function (button) {
        // Prevent loopback.
        var promise = this._promise;
        if (!promise) {
            this.dispose();
        ...
        var body = this._body;
        var value = null;
        if (button.accept && body instanceof widgets_1.Widget && typeof body.getValue === 'function') {
            value = body.getValue();
        }
        this.dispose();
        promise.resolve({ button: button, value: value });
    };
    return Dialog;
}(widgets_1.Widget));
    ...
    var Renderer = /** @class */ (function () {
        ...
        Renderer.prototype.createHeader = function (title) {
            ...
            if (typeof title === 'string') {
                ...
                header = new widgets_1.Widget({ node: virtualdom_1.VirtualDOM.realize(title) });
            }
        ...
        Renderer.prototype.createBody = function (value) {
            ...
            if (typeof value === 'string') {
                ...
                body = new widgets_1.Widget({ node: virtualdom_1.VirtualDOM.realize(value) });
            }
        ...
        Renderer.prototype.renderIcon = function (data) {
            return virtualdom_1.h.div({ className: this.createIconClass(data) }, data.iconLabel);
        };
        /**
...
(function (Private) {
    ...
    function handleOptions(options) {
        if (options === void 0) { options = {}; }
        var buttons = (options.buttons || [Dialog.cancelButton(), Dialog.okButton()]);
        var defaultButton = buttons.length - 1;
