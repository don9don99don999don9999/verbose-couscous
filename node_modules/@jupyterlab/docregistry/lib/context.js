...
var Context = /** @class */ (function () {
    ...
    function Context(options) {
        ...
        var localPath = this._manager.contents.localPath(this._path);
        var lang = this._factory.preferredLanguage(coreutils_2.PathExt.basename(localPath));
        var dbFactory = options.modelDBFactory;
        if (dbFactory) {
            var localPath_1 = manager.contents.localPath(this._path);
            this._modelDB = dbFactory.createNew(localPath_1);
    ...
    Object.defineProperty(Context.prototype, "localPath", {
        /**
    ...
    Context.prototype.saveAs = function () {
        ...
        return Private.getSavePath(this._path).then(function (newPath) {
            ...
            if (newPath === _this._path) {
                return _this.save();
            }
            // Make sure the path does not exist.
            return _this._manager.ready.then(function () {
                return _this._manager.contents.get(newPath);
            }).then(function () {
                return _this._maybeOverWrite(newPath);
            }).catch(function (err) {
                if (!err.response || err.response.status !== 404) {
                    throw err;
                }
                return _this._finishSaveAs(newPath);
            });
        });
    };
    /**
     ...
     */
    Context.prototype.addSibling = function (widget, options) {
    ...
    Context.prototype._onFileChanged = function (sender, change) {
        ...
        if (newPath && oldPath === this._path) {
            ...
            var localPath = this._manager.contents.localPath(newPath);
     ...
     */
    Context.prototype._onSessionChanged = function (sender, type) {
        if (type !== 'path') {
            return;
        }
    ...
    Context.prototype._populate = function () {
        ...
        return this._maybeCheckpoint(false).then(function () {
            if (_this.isDisposed) {
                return;
            }
     ...
     */
    Context.prototype._handleError = function (err, title) {
        ...
        if (err instanceof services_1.ServerConnection.ResponseError) {
            err.response.text().then(function (text) {
                ...
                apputils_1.showDialog({ title: title, body: body, buttons: buttons });
            });
        }
        else {
     ...
     */
    Context.prototype._maybeCheckpoint = function (force) {
        var _this = this;
        var writable = this._contentsModel && this._contentsModel.writable;
        ...
        if (force) {
            ...
            promise = this.listCheckpoints().then(function (checkpoints) {
                writable = _this._contentsModel && _this._contentsModel.writable;
                if (!_this.isDisposed && !checkpoints.length && writable) {
     ...
     * Handle a time conflict.
     */
    Context.prototype._maybeOverWrite = function (path) {
        var _this = this;
        var body = "\"" + path + "\" already exists. Do you want to replace it?";
        var overwriteBtn = apputils_1.Dialog.warnButton({ label: 'OVERWRITE' });
        return apputils_1.showDialog({
            title: 'File Overwrite?', body: body,
            ...
            if (result.button.label === 'OVERWRITE') {
                return _this._manager.contents.delete(path).then(function () {
                    _this._finishSaveAs(path);
                });
            }
        });
    };
     ...
     */
    Context.prototype._finishSaveAs = function (newPath) {
        var _this = this;
        this._path = newPath;
        return this.session.setPath(newPath).then(function () {
            _this.session.setName(newPath.split('/').pop());
            return _this.save();
        }).then(function () {
            _this._pathChanged.emit(_this._path);
            return _this._maybeCheckpoint(true);
        });
    };
    return Context;
}());
